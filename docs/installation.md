...
Follow these steps to set up the HSClassifier project on your local machine.
...
```bash
git clone https://github.com/your-username/hsclassifier.git
cd hsclassifier
```
...
This will fetch and install all the Go modules required for the project.


## Step 3: Install Python Dependencies

If you need to install Python dependencies, you can use the** **`requirements.txt` file provided in the project:
...
This will install all the necessary Python packages, such as** **`pandas` and others required by the Python scripts.

## Step 4: Generate Sample Data (Optional)

If you want to generate sample data for testing purposes, run the** **`create_sample_data.py` script:
...
This will generate the** **`data/descriptions.xlsx` file used by the project.

## Step 5: Verify the Installation

To verify the setup, you can run the Go application and ensure everything is working correctly. Run the following command:
...
If the setup is successful, the project should process the data and categorize descriptions.
